...
export function OnboardingTooltip({ id, children, position = 'bottom' }: OnboardingTooltipProps) {
  ...
  function handleDismiss() { setVisible(false); dismissTooltip(id); }
  return (
    <div className={`absolute z-40 ${position === 'top' ? 'bottom-full mb-2' : 'top-full mt-2'} left-0 right-0 animate-fade-in`}>
      <div className="bg-accent/95 text-page-bg text-xs rounded-lg px-3 py-2.5 shadow-lg max-w-sm mx-auto flex items-start gap-2">
        ...
        <button onClick={handleDismiss} className="shrink-0 opacity-70 hover:opacity-100 transition-opacity mt-0.5"><X size={12} /></button>
      </div>
    </div>
  );
}
...
export function useTooltipDismissed(id: string): boolean {
  ...
  useEffect(() => { setDismissed(getDismissed().has(id)); }, [id]);
  return dismissed;
}
